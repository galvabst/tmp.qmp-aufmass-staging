...
-- 1. Add nur_fuer_neue column to contractor_akademie_lektionen
ALTER TABLE thermocheck.contractor_akademie_lektionen
  ADD COLUMN nur_fuer_neue BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN thermocheck.contractor_akademie_lektionen.nur_fuer_neue IS
  'Wenn true, ist diese Lektion nur für neue Onboarder relevant. Fertige Techniker (ready) werden nicht zum Nachholen gezwungen.';

-- 2. Replace the admin_upsert_akademie_lektion RPC to handle the new field
...
BEGIN
  -- Admin check
  IF NOT thermocheck.is_admin() THEN
...
-- 3. Public wrapper (must exist for frontend calls)
...
AS $$
  SELECT thermocheck.admin_upsert_akademie_lektion(p_data);
$$;
