...
DECLARE
  ...
  v_produkt_typ thermocheck.contractor_bestellung_produkt_typ_enum;
  v_idem text;
  v_new_id uuid;
  v_action text;
BEGIN
  ...
  -- Enum-Cast über Text: contractor_produkt_typ → contractor_bestellung_produkt_typ_enum
  v_produkt_typ := v_produkt.produkt_typ::text::thermocheck.contractor_bestellung_produkt_typ_enum;

  -- 1) Bereits paid mit derselben Sub/PI?
  ...
  -- 2) Bestehende pending/failed Zeile updaten
  SELECT id INTO v_existing_id
  FROM thermocheck.contractor_bestellungen
  WHERE onboarding_id = p_onboarding_id
  ...
  ELSE
    -- 3) Neue Zeile anlegen (idempotent)
    ...
    VALUES (
      p_onboarding_id, v_produkt_typ, p_produkt_key,
